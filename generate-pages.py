...
PACKAGES_LIST = ["pydantic", "numpy", "pillow", "scipy", "tiktoken", "tokenizers"]
...
def download_file(url, destination):
    ...
    try:
        ...
        ) as r:  # Use stream=True, add timeout
...
""")
                ...
                for wheel_name, wheel_url in sorted(wheels_info):
                    ...
                    if "linux_aarch64" in wheel_name and (
                        ...
                        if not os.path.exists(local_wheel_path):
                            if not download_file(wheel_url, local_wheel_path):
                                print(
                                    f"ERROR: Failed to download/save {display_name}. Skipping link generation for this file."
                                )
                    ...
                    package_index.write(
                        f'    <a href="{link_url}"{hash_part}>{display_name}</a><br/>\n'
                    )
                # --- MODIFICATION END ---
                package_index.write("""
</body>
...
""")
    ...
    if skipped_due_download_error > 0:
        print(
            f"Warning: Skipped adding links for {skipped_due_download_error} files due to download errors."
        )
...
""")
            ...
            for package_name in sorted(packages):
                ...
                main_package_index.write(
                    f'    <a href="{normalized_package_name}/">{normalized_package_name}</a><br/>\n'
                )
            main_package_index.write("""
</body>
</html>
...
def main():
    ...
    print(
        f"Make sure to commit and push the entire '{DOCS_DIR}' directory (including downloaded wheels) to your GitHub Pages branch."
    )
